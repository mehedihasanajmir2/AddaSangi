...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, user, onProfileClick }) => {
  const menuItems = [
  ...
  return (
    <aside className="hidden md:flex flex-col w-[280px] p-4 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
      ...
      {menuItems.map(item => (
        ...
        >
          <div className={`w-8 flex justify-center text-xl ${activeTab === item.id ? item.color : 'text-gray-600 group-hover:scale-110 transition-transform'}`}>
